...
inline std::vector<InstanceData> GenerateRandomInstances(int count, const glm::vec3 &boundsMin,
                                                         const glm::vec3 &boundsMax, const glm::vec3 &safeZoneMin,
                                                         const glm::vec3 &safeZoneMax)
{
    ...
    auto isInSafeZone = [&](const glm::vec3 &pos) {
        return pos.x >= safeZoneMin.x && pos.x <= safeZoneMax.x && pos.y >= safeZoneMin.y && pos.y <= safeZoneMax.y &&
               pos.z >= safeZoneMin.z && pos.z <= safeZoneMax.z;
    };

    while (static_cast<int>(instances.size()) < count)
    {
        glm::vec3 position(distX(gen), distY(gen), distZ(gen));

        if (isInSafeZone(position))
        {
            continue; // reject and re-roll — too close to the origin
        }
